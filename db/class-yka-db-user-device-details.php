...
class YKA_DB_USER_DEVICE_DETAILS extends YKA_DB_BASE{
  ...
  function deviceIDExists( $device_id, $user_id ){
    global $wpdb;
    $table_name = $this->getTable();
    $rs = (int) $this->get_var( "SELECT COUNT(user_id) FROM $table_name WHERE user_id = $user_id AND device_id = '$device_id';" );
    if( !$rs ){
      return false;
    }
    return true;
  ...
  function getUserDeviceDetails( $user_id ){
    ...
    $data = array();
    $table_name = $this->getTable();
    $query = $wpdb->prepare( "SELECT device_id, fcm_token, login_status FROM $table_name WHERE user_id = %d", array( $user_id ) );
